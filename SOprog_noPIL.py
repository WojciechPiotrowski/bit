...
class window:
        ...
        def __init__(self, Frame, func, State):
            ...
            def go():
                ...
                if state == 'Finished':
                    ...
                    self.Button.config(text='Finished with output')
                    # self.Button.config(image=self.img5)
                    State.set(state + ' in ' + details)

                elif state == 'ERROR':
                    self.Button.config(text='ERROR')
            ...
            self.Button.config(text='GO!')
            # self.Button.config(image=self.img1)
            self.Button['border'] = '2'
    ...
    class open_file_button:
        def __init__(self, Frame):
            ...
            self.Button = tk.Button(Frame, text='FILE', command=lambda: self.filepath())
    ...
    class choose_dir_button:
        def __init__(self, Frame):
            ...
            self.Button = tk.Button(Frame, text='FOLDER', command=lambda: self.dirpath())
            # self.Button.config(image=self.img)
    ...
    def mus(self):
        ...
        But11 = self.open_file_button(Frame2)
    ...
    def hiidenmark(self):
        def goHierarchiesDenmark():
            import HiiDenmark
            return HiiDenmark.main(self.period, self.country, But1.DirPath.get())
        def setstandardpaths():
            try:
                But1.DirPath.set(
                    master_file.dir_dictionary('Hierarchies', self.country, calc_week(self.period)))
            except:
                showmessage('ERROR', 'Please set period and country correctly')
        ...
        Lbl1 = tk.Label(Frame2, text='Choose Hierarchies folder')
        Lbl1.grid(row=1, column=0)
